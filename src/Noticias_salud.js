...
export function Noticias_salud () {
    ...
    useEffect(() => {
        const fetchNoticias_salud = async () => {
            try {
                const response = await axios.get(`https://gnews.io/api/v4/top-headlines?category=health&lang=es&apikey=0916952f5281314fd60dcb8b93736b61`);
        ...
        fetchNoticias_salud(); // Llamar a la función para obtener datos
    ...
    return (        
        ...
        <tr>
              <td align="center" colspan="2"><span className='valor'>NOTICIAS SALUD</span></td>
        </tr>
        {titulos.map(titulo => (
          <tr key={titulo.id}>
            <td width="50%">
