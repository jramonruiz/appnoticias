...
export function Noticias_ciencia () {
    ...
    useEffect(() => {
        const fetchNoticias_ciencia = async () => {
            try {
                const response = await axios.get(`https://gnews.io/api/v4/top-headlines?category=science&lang=es&apikey=0916952f5281314fd60dcb8b93736b61`);
        ...
        fetchNoticias_ciencia(); // Llamar a la función para obtener datos
    ...
    return (        
        ...
        <tr>
              <td align="center" colspan="2"><span className='valor'>NOTICIAS CIENCIA</span></td>
        </tr>
        {titulos.map(titulo => (
          <tr key={titulo.id}>
            <td width="50%">
...
export default Noticias_ciencia